...
// Implementation
CShaderTextured3D::CShaderTextured3D( const int _kiShaderID, const std::wstring _kstrPath ) :
	CShader( _kiShaderID, _kstrPath )
{
	this->m_bIsInitialised = true;

	this->m_pEffect = CreateEffect( _kstrPath + L"textured_shader_3d.fx" );
...
ID3D10EffectMatrixVariable* CShaderTextured3D::GetVarWVP()
{
	return ( m_pVarWVP );
}

ID3D10EffectShaderResourceVariable* CShaderTextured3D::GetVarTexture()
{
