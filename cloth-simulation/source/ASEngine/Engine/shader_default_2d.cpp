...
// Implementation
CShaderDefault2D::CShaderDefault2D( const int _kiShaderID, const std::wstring _kstrPath ) : 
	CShader( _kiShaderID, _kstrPath )
{
	this->m_bIsInitialised = true;

	this->m_pEffect = CreateEffect( _kstrPath + L"default_shader_2d.fx" );
...
ID3D10EffectMatrixVariable* CShaderDefault2D::GetVarWVP()
{
	return ( m_pVarWVP );
}