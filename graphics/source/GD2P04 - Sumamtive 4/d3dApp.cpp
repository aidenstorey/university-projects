...
int D3DApp::run()
{
	...
	while(msg.message != WM_QUIT)
	{
        ...
        {	
			...
			if( !mAppPaused )
			{
				updateScene(mTimer.getDeltaTime(),mTimer.getGameTime());	
				mTimeElapsed += mTimer.getDeltaTime();
			}	
			else
			{
				Sleep(50);
...
void D3DApp::initApp()
{
    ...
    wcscpy_s(fontDesc.FaceName, L"Courier New");
...
void D3DApp::updateScene( float dt, float gt )
{
	...
	if( (mTimer.getGameTime() - t_base) >= 1.0f )
	{
		...
		outs.precision(6);
		outs << L"FPS: " << fps << L"\nW/A/S/D or Arrow Keys - Movement\nSpace - Ascend\nShift - Descend\nR - Reset demo\nT - Toggle rain\nG - Speed up Day/night cycle\nE - Toggle cube for reflection";
